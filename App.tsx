...
const BackgroundEffects = () => {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden select-none z-0 bg-black">
          ...
          >
              <div className="relative w-48 h-12">
                   ...
                   <div className="absolute inset-0 bg-gradient-to-b from-gray-300 via-gray-400 to-gray-700 rounded-[50%] shadow-2xl border border-gray-500 overflow-hidden flex items-center">
                        {/* Rotating Lights Strip */}
                        <div className="w-[200%] h-2 flex gap-4 animate-ufo-lights-spin">
                            {Array.from({length: 20}).map((_, i) => (
                                <div key={i} className={`w-3 h-3 rounded-full ${i % 3 === 0 ? 'bg-red-500' : (i % 3 === 1 ? 'bg-green-500' : 'bg-blue-500')} shadow-[0_0_5px_currentColor]`} />
                            ))}
                        </div>
                   </div>

                   {/* Underglow Engine */}
                   <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-24 h-4 bg-green-500/50 blur-md rounded-full animate-pulse" />
              </div>
          </div>
      )}
      ...
      {/* Lens Flare / Warzone Haze - Enhanced with Top-Left Lens Flare */}
      <div className="absolute inset-0 z-30 pointer-events-none mix-blend-screen overflow-hidden">
          {/* Main Flare Source (Top Left) */}
          <div className="absolute -top-[10%] -left-[10%] w-[50%] h-[50%] bg-[radial-gradient(circle,rgba(255,255,255,0.8)_0%,rgba(200,230,255,0.3)_30%,rgba(0,0,0,0)_70%)] blur-[60px] animate-pulse" style={{ animationDuration: '4s' }} />
          
          {/* Flare Artifacts (Projecting towards center) */}
          <div className="absolute top-[15%] left-[15%] w-32 h-32 rounded-full bg-blue-400/10 blur-xl" />
          <div className="absolute top-[25%] left-[25%] w-16 h-16 rounded-full bg-purple-400/10 blur-md" />
          <div className="absolute top-[40%] left-[40%] w-8 h-8 rounded-full bg-teal-400/20 blur-sm" />

          {/* Existing Haze */}
          <div className="absolute top-[-20%] left-[-20%] w-[60%] h-[60%] bg-[radial-gradient(circle,rgba(6,182,212,0.15)_0%,transparent_70%)] blur-[80px]" />
          {/* Subtle Scanlines */}
    ...
    if (!document.getElementById('anim-styles')) {
        ...
        style.innerHTML = `
            ...
            .animate-text-shimmer {
                background: linear-gradient(90deg, #94a3b8 0%, #ffffff 20%, #fbbf24 50%, #ffffff 80%, #94a3b8 100%);
    ...
    <div className="h-screen w-screen overflow-hidden bg-black text-gray-200 font-sans select-none relative" onMouseUp={(e) => handleMouseUp(e.nativeEvent)}>
        ...
        <div className="absolute top-0 left-0 right-0 z-50 h-16 bg-gray-900/80 backdrop-blur-md border-b border-gray-700 flex items-center justify-between px-6 shadow-2xl">
            ...
            <div className="flex items-center gap-6">
                <h1 className="text-xl font-black italic tracking-tighter animate-text-shimmer select-none whitespace-nowrap">
            ...
            <div className="flex items-center gap-6">
                 <div className="flex items-center gap-3 bg-black/40 px-3 py-1 rounded-lg border border-gray-700/50">
                    ...
                    <div className="flex flex-col">
                        <span className="text-[9px] text-gray-500 font-bold uppercase">Spins</span>
                        <input type="number" value={maxSpins} onChange={e => setMaxSpins(Number(e.target.value))} className="w-12 bg-transparent text-white font-mono text-xs font-bold outline-none border-b border-gray-600 focus:border-blue-500" />
                    </div>
                    <div className="flex flex-col">
                        <span className="text-[9px] text-gray-500 font-bold uppercase">Runs</span>
                        <input type="number" value={numSimulations} onChange={e => setNumSimulations(Math.min(100, Math.max(1, Number(e.target.value))))} className="w-10 bg-transparent text-white font-mono text-xs font-bold outline-none border-b border-gray-600 focus:border-blue-500" />
                    </div>
                    <div className="w-[1px] h-6 bg-gray-700" />
                     <div className="flex flex-col">
                        <span className="text-[9px] text-gray-500 font-bold uppercase text-red-400">Stop Loss</span>
                        <input type="number" value={stopLoss} onChange={e => setStopLoss(Number(e.target.value))} className="w-14 bg-transparent text-white font-mono text-xs font-bold outline-none border-b border-gray-600 focus:border-red-500" />
                    </div>
                    <div className="flex flex-col">
                        <span className="text-[9px] text-gray-500 font-bold uppercase text-green-400">Take Profit</span>
                        <input type="number" value={takeProfit} onChange={e => setTakeProfit(Number(e.target.value))} className="w-14 bg-transparent text-white font-mono text-xs font-bold outline-none border-b border-gray-600 focus:border-green-500" />
                    </div>
                 </div>
